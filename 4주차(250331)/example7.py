...
img = cv.imread('data/soccer.jpg')  # 영상 읽기
img_show = np.copy(img)  # 붓칠을 디스플레이할 목적의 영상

mask = np.zeros((img.shape[0], img.shape[1]), np.uint8)  # 모든 화소를 배경일 것 같음으로 초기화
mask[:] = cv.GC_PR_BGD
...
while True:
    cv.imshow('Painting', img_show) # 이거 추가해야 칠해지는 부분 실시간으로 보임 ## jh
    if cv.waitKey(1) == ord('q'):  # 붓칠을 끝내려면 'q' 키 누름
        break
